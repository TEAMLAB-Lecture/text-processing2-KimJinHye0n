...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    """
    string_list = [i for i in input_string if i >= '0' and i <= '9']
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
    ...
    camelcase_list = []
    camelcase_list.append(underscore_str[0].lower())
    for i in range(1, len(underscore_str)) :
        if underscore_str[i - 1] == '_' :
            camelcase_list.append(underscore_str[i].upper())
        else :
            camelcase_list.append(underscore_str[i].lower()) 
    camelcase_str = ''.join([i for i in camelcase_list if i != '_'])
